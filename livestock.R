...
library(dplyr)#to produce joins
library(sf)
#2.- importing livestock data tables and combining it in one file.
livestock_files <- list.files("Tables/LELF_2005 - 2018/", pattern = ".xlsx", full.names = TRUE)
livestock.list <- list()
...
#2.2.-saving 
save(livestock.list,file = "rda/livestock.list.rda")
...
for (i in 1:length(s_maize_post2007)) {
  s_maize_post2007[[i]]$area <-st_area(s_maize_post2007[[i]])#in square meters
}
#4.2.-Adjusting projections
for( i in 1:11){
  s_maize_post2007[[i]]<-st_transform(s_maize_post2007[[i]],32633)
}
#4.3-intersection
#it creates only the area where the polygons intersect
load("rda/buffer_list.rda")
s_maize_BNR_ZD_intersect <- list()
for (i in 1:length(s_maize_post2007)) {
  s_maize_BNR_ZD_intersect[[i]] <- st_intersection(s_maize_post2007[[i]],buffer.list[[i]])
}

#4.4.-saving joining
save(s_maize_BNR_ZD_intersect,file = "rda/s_maize_BNR_ZD_intersect.rda")
...
for (j in 1:length(s_maize_post2007)) {
  s_maize_BNR_ZDlist[[j]]<-as.data.frame(s_maize_BNR_ZD_intersect[[j]][,c("BNR_ZD","area")])
  s_maize_BNR_ZD_aggr[[j]]<- aggregate(x=s_maize_BNR_ZDlist[[j]]["area"],
                                         by=s_maize_BNR_ZDlist[[j]]["BNR_ZD"],
                                         FUN=sum)
}

#4.6.-saving aggregate
save(s_maize_BNR_ZD_aggr,file = "rda/s_maize_BNR_ZD_aggr.rda")
...
for (j in 1:length(s_maize_post2007)) {
  s.maize_post2007list[[j]]<-as.data.frame(s_maize_post2007[[j]][,c("BNR_ZD","area")])
  s_maize_post2007_aggr[[j]]<- aggregate(x=s.maize_post2007list[[j]]["area"],
                                         by=s.maize_post2007list[[j]]["BNR_ZD"],
                                         FUN=sum)
}

#5.2.-saving total aggregate
save(s_maize_post2007_aggr,file = "rda/s_maize_post2007_aggr.rda")
#load("rda/s_maize_post2007_aggr.rda")

#6.-joining both areas and Proportion
livestock_BNR_ZD<-list()
for (i in 1:length(s_maize_post2007_aggr)){
  livestock_BNR_ZD[[i]]<-left_join(s_maize_post2007_aggr[[i]],s_maize_BNR_ZD_aggr[[i]],by="BNR_ZD")
  names(livestock_BNR_ZD[[i]])<- c("BNR_ZD","area_Farm","area_buffer")
...
#8.2- joining table and calculate score (cat_liv)
for (i in 1:length(livestock_post2007)){
  livestock_post2007[[i]]$`BNR-ZD`<-as.factor(livestock_post2007[[i]]$`BNR-ZD`)
  livestock_BNR_ZD[[i]]<-left_join(livestock_post2007[[i]],livestock_BNR_ZD[[i]],by=c("BNR-ZD"="BNR_ZD"))
  livestock_BNR_ZD[[i]]$Consumptionarea<-livestock_BNR_ZD[[i]]$Rinder*consumption_factor$Consumption.factor[[i]]#in hectares
  ...
  livestock_BNR_ZD[[i]]$cat_liv<-ifelse(is.na(livestock_BNR_ZD[[i]]$cal),livestock_BNR_ZD[[i]]$cat_liv,livestock_BNR_ZD[[i]]$cal)
  livestock_BNR_ZD[[i]]$cat_liv<-ifelse(livestock_BNR_ZD[[i]]$cat_liv<0,0,livestock_BNR_ZD[[i]]$cat_liv)
  livestock_BNR_ZD[[i]]<-livestock_BNR_ZD[[i]]%>%select("BNR-ZD","cat_liv")
  livestock_BNR_ZD[[i]]$`BNR-ZD`<-as.factor(livestock_BNR_ZD[[i]]$`BNR-ZD`)
}

#8.3- save 
save(livestock_BNR_ZD, file = "rda/livestock_BNR_ZD.rda")
#load("rda/livestock_BNR_ZD.rda")

#9.-loading silage maize data
load("rda/silage_maize_cat.rda")

#10.-joining the cat_livs to the silage maize selected
for (i in 1:length(silage_maize_cat)){
  silage_maize_cat[[i]]<-left_join(silage_maize_cat[[i]],livestock_BNR_ZD[[i]],by=c("BNR_ZD"="BNR-ZD"))
}
